<template>
    <SignIn title="Sign in to your account">
        <form class="mt-8 space-y-6" @submit.prevent="authStore.handleLogin(form)">
          ...
          <div class="-space-y-px rounded-md shadow-sm">
            ...
            <div>
              ...
              <input id="password" name="password" v-model="form.password" type="password" autocomplete="current-password" required="" class="relative block w-full appearance-none rounded-none rounded-b-md border border-gray-300 px-3 py-2 text-gray-900 placeholder-gray-500 focus:z-10 focus:border-indigo-500 focus:outline-none focus:ring-indigo-500 sm:text-sm" placeholder="Password" />
            </div>
          </div>
